import os, sqlite3, uuid, shutil, zipfile, io, threading
try:
...
def init_db():
    with get_db() as db:
        ...
        try: db.execute('ALTER TABLE photos ADD COLUMN approved_at DATETIME')
        except: pass
        try: db.execute('ALTER TABLE photos ADD COLUMN sort_order INTEGER DEFAULT 0')
        except: pass
        db.execute('''CREATE TABLE IF NOT EXISTS config(key TEXT PRIMARY KEY, value TEXT)''')
        for k,v in [('event_name','Photo Wall'),('mod_password','admin123'),
                    ('theme','fiesta'),('font','grande')]:
...
def get_theme_parts(theme):
    ...
    if theme == 'boda':
        css_vars = '--c1:#E8C4C4;--c2:#F9F3EF;--c3:#B07C9E;--dark:#120f14;'
        bg_css = '''
body{background:#120f14}
body::before{content:'';position:fixed;inset:0;
  background:radial-gradient(ellipse at 30% 40%,rgba(232,196,196,.22) 0%,transparent 55%),
             radial-gradient(ellipse at 70% 60%,rgba(176,124,158,.15) 0%,transparent 55%),
             radial-gradient(ellipse at 50% 80%,rgba(249,243,239,.07) 0%,transparent 50%);
  pointer-events:none}'''
        particles_js = '''
...
<svg style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none;opacity:.07;z-index:0" viewBox="0 0 800 500" preserveAspectRatio="xMidYMid slice">
  <text x="400" y="280" text-anchor="middle" font-size="320" fill="#E8C4C4" font-family="serif">💍</text>
</svg>'''
...
def upload_page():
    ...
    grad = {'boda':'#E8C4C4,#F9F3EF','cumple':'#FF6B6B,#FFE66D',
            'quince':'#C084FC,#F472B6','empresarial':'#38BDF8,#e2e8f0',
            'fiesta':'#FF3CAC,#FFD700'}.get(theme,'#FF3CAC,#FFD700')
    btn_grad = {'boda':'#E8C4C4,#B07C9E','cumple':'#FF6B6B,#4ECDC4',
...
.pc.drag-over{{border-color:#FFD700;background:rgba(255,215,0,.08);transform:scale(1.02)}}
.pc.dragging{{opacity:.4;transform:scale(0.97)}}
.drag-handle{{font-size:.75rem;color:rgba(255,255,255,.2);text-align:center;padding:4px 0;cursor:grab;letter-spacing:3px}}
.drag-handle:active{{cursor:grabbing}}
.pc img{{width:100%;object-fit:contain;display:block;background:#000;max-height:220px}}
.pcb{{padding:10px}}
...
.brej{{padding:6px;background:rgba(255,60,60,.07);border:1px solid rgba(255,60,60,.25);color:#ff6b6b;border-radius:6px;cursor:pointer;font-size:.8rem;font-weight:500;transition:all .2s}}
.brej:hover{{background:rgba(255,60,60,.16)}}
.bpend{{flex:1;padding:6px;background:rgba(255,200,0,.07);border:1px solid rgba(255,200,0,.25);color:#FFD700;border-radius:6px;cursor:pointer;font-size:.78rem;font-weight:500;transition:all .2s}}
.bpend:hover{{background:rgba(255,200,0,.18)}}
...
<div class="content">
  ...
  <div class="tc cfg {'on' if open_tab=='config' else ''}" id="tc-config">
    ...
    <div class="section" style="margin-top:16px">
      <h2>📥 Descargar fotos del evento</h2>
      <p style="color:rgba(255,255,255,.4);font-size:.84rem;margin-bottom:12px">Descargá un ZIP con todas las fotos aprobadas del evento.</p>
      <a href="/api/download-zip" target="_blank"><button type="button" class="bsave" style="background:linear-gradient(135deg,#2B86C5,#784BA0)">📥 DESCARGAR ZIP</button></a>
    </div>
...
function card(p,tab){{
  const acts=tab==='pending'
    ...
    :`<button class="bpend" onclick="act(${{p.id}},'unpublish')" title="Volver a pendientes">↩ Pendientes</button><button class="brej" onclick="act(${{p.id}},'reject')">✗ Borrar</button>`;
  const handle=tab==='approved'?`<div class="drag-handle" title="Arrastrá para reordenar">⠿ ⠿ ⠿</div>`:'';
...
async function load(){{
  ...
  loading=true;
  try{{
    const r=await fetch('/api/photos-with-counts?status='+cur);
    const data=await r.json();
    const photos=data.photos||[];
    document.getElementById('cnt').textContent=data.pending+' pend · '+data.approved+' en pantalla';
    ...
    if(cur==='approved'){{
      photos.forEach(p=>{{const el=document.getElementById('c'+p.id);if(el)initDrag(el,p.id);}});
    }}
...
async function act(id,a){{
  ...
  // If unpublishing, switch to pending tab to show it
  if(a==='unpublish'){{
    const pendTab=document.querySelector('.tab');
    if(pendTab)sw('pending',pendTab);
  }} else {{
    load();
  }}
}}
...
let dragId=null,dragEl=null,placeholder=null,startY=0,startX=0;

function initDrag(el,id){{
  el.addEventListener('mousedown',e=>startDrag(e,id,el));
  el.addEventListener('touchstart',e=>startDrag(e,id,el),{{passive:true}});
}}
...
function startDrag(e,id,el){{
  ...
  placeholder.style.cssText='height:'+rect.height+'px;border:2px dashed #FFD700;border-radius:12px;background:rgba(255,215,0,.08);margin-bottom:0;box-sizing:border-box;';
  el.parentNode.insertBefore(placeholder,el.nextSibling);

  // Float the dragged card
  el.style.cssText+='position:fixed;z-index:999;width:'+rect.width+'px;top:'+(clientY-startY)+'px;left:'+(clientX-startX)+'px;opacity:.85;pointer-events:none;box-shadow:0 8px 32px rgba(0,0,0,.5);';
...
function onMove(e){{
  ...
  dragEl.style.left=(clientX-startX)+'px';
  ...
  dragEl.style.pointerEvents='none';
  if(!under)return;
  const card=under.closest('.pc');
  if(card&&card!==dragEl&&card.id!==('c'+dragId)){{
    const grid=document.getElementById('g-approved');
    const cards=[...grid.querySelectorAll('.pc:not([style*="position:fixed"])')];
    const idx=cards.indexOf(card);
    if(idx>-1){{
      const rect=card.getBoundingClientRect();
      const mid=rect.top+rect.height/2;
      if(clientY<mid){{grid.insertBefore(placeholder,card);}}
...
async function onDrop(e){{
  ...
  dragEl.style.top='';dragEl.style.left='';dragEl.style.opacity='';
  dragEl.style.pointerEvents='';dragEl.style.boxShadow='';
  ...
  const grid=document.getElementById('g-approved');
  const newOrder=[...grid.querySelectorAll('.pc')].map(c=>parseInt(c.id.replace('c','')));
  ...
  dragEl=null;dragId=null;placeholder=null;
  load();
}}
...
def api_approved_display():
    with get_db() as db:
        photos=db.execute('SELECT * FROM photos WHERE status="approved" ORDER BY sort_order ASC, approved_at ASC').fetchall()
    return jsonify([dict(p) for p in photos])



def compress_in_background(filepath):
    """Compress photo in background thread — doesn't block approve response."""
    if not HAS_PIL: return
    try:
        from PIL import ImageOps as _IO
        with PILImage.open(filepath) as img:
            img = img.convert('RGB')
            try: img = _IO.exif_transpose(img)
            except: pass
            w, h = img.size
            if max(w,h) > 1920:
                ratio = 1920 / max(w,h)
...
def approve_photo(pid):
    with get_db() as db:
        ...
        max_order = db.execute("SELECT COALESCE(MAX(sort_order),0) FROM photos WHERE status='approved'").fetchone()[0]
        db.execute('UPDATE photos SET status="approved", approved_at=CURRENT_TIMESTAMP, sort_order=? WHERE id=?',
                   (max_order+1, pid))
        db.commit()
    # Compress in background — returns immediately without waiting
    if photo and HAS_PIL:
        fp = os.path.join(UPLOAD_FOLDER, photo['filename'])
        if os.path.exists(fp):
            threading.Thread(target=compress_in_background, args=(fp,), daemon=True).start()
    return jsonify({'ok':True})


@app.route('/api/photos/<int:pid>/unpublish', methods=['POST'])
@login_required
def unpublish_photo(pid):
    with get_db() as db:
        db.execute('UPDATE photos SET status="pending", approved_at=NULL, sort_order=0 WHERE id=?', (pid,))
        db.commit()
    return jsonify({'ok': True})
...
# Debug route - check if Pillow works on server
@app.route('/debug/compression')
def debug_compression():
    info = {'HAS_PIL': HAS_PIL, 'PIL_version': None}
    if HAS_PIL:
        import PIL, io
        info['PIL_version'] = PIL.__version__
        try:
            test = PILImage.new('RGB', (3000,4000), color=(100,150,200))
            buf_before = io.BytesIO()
            test.save(buf_before, 'JPEG', quality=100)
            w, h = test.size
            max_dim = max(w, h)
            if max_dim > 1920:
                ratio = 1920 / max_dim
                test = test.resize((int(w*ratio), int(h*ratio)), PILImage.LANCZOS)
            buf_after = io.BytesIO()
            test.save(buf_after, 'JPEG', quality=82, optimize=True)
            info['test_before_kb'] = len(buf_before.getvalue()) // 1024
            info['test_after_kb'] = len(buf_after.getvalue()) // 1024
            info['compression_test'] = 'OK'
        except Exception as e:
            info['compression_test'] = 'FAIL: ' + str(e)
    return jsonify(info)

# Reorder photos in carousel — accepts full new order
@app.route('/api/reorder-all', methods=['POST'])
@login_required
def reorder_photos_all():
    ...
    with get_db() as db:
        for i, pid in enumerate(order):
            db.execute('UPDATE photos SET sort_order=? WHERE id=?', (i, pid))
        db.commit()
    return jsonify({'ok': True})
...
def reorder_photos():
    data = request.get_json()
    drag_id   = int(data.get('drag_id', 0))
    target_id = int(data.get('target_id', 0))
    if not drag_id or not target_id:
        return jsonify({'error': 'Missing ids'}), 400
    with get_db() as db:
        drag   = db.execute('SELECT sort_order FROM photos WHERE id=?', (drag_id,)).fetchone()
        target = db.execute('SELECT sort_order FROM photos WHERE id=?', (target_id,)).fetchone()
        if not drag or not target:
            return jsonify({'error': 'Not found'}), 404
        d_ord = drag['sort_order']
        t_ord = target['sort_order']
        if d_ord < t_ord:
            db.execute(
                'UPDATE photos SET sort_order=sort_order-1 WHERE sort_order>? AND sort_order<=? AND status="approved"',
                (d_ord, t_ord))
        else:
            db.execute(
                'UPDATE photos SET sort_order=sort_order+1 WHERE sort_order>=? AND sort_order<? AND status="approved"',
                (t_ord, d_ord))
        db.execute('UPDATE photos SET sort_order=? WHERE id=?', (t_ord, drag_id))
        db.commit()
    return jsonify({'ok': True})

# ══════════════════════════════════════════════════════════
# PHOTOS + COUNTS — single endpoint to reduce requests
# ══════════════════════════════════════════════════════════
@app.route('/api/photos-with-counts')
@login_required
def api_photos_with_counts():
    status = request.args.get('status', 'pending')
    with get_db() as db:
        photos   = db.execute('SELECT * FROM photos WHERE status=? ORDER BY uploaded_at DESC' if status=='pending' else 'SELECT * FROM photos WHERE status=? ORDER BY sort_order ASC, approved_at ASC', (status,)).fetchall()
    ...
    return jsonify({'photos':[dict(p) for p in photos], 'pending':pending, 'approved':approved})
...
.swiper-slide{{display:flex;align-items:center;justify-content:center;overflow:hidden}}
.swiper-slide img{{width:100%;height:100%;object-fit:contain;will-change:transform;transform:scale(1);transform-origin:center center}}
.topbar{{position:absolute;top:0;left:0;right:0;height:clamp(42px,6vh,62px);z-index:5;display:flex;align-items:center;justify-content:space-between;padding:0 clamp(10px,2vw,22px);padding-left:clamp(50px,7vw,110px);background:linear-gradient(to bottom,rgba(0,0,0,.7),transparent);pointer-events:none}}
...
.banner{{position:absolute;bottom:0;left:0;right:0;height:clamp(38px,5.5vh,58px);background:rgba(0,0,0,.88);backdrop-filter:blur(16px);border-top:1px solid rgba(255,255,255,.1);display:flex;align-items:center;justify-content:center;z-index:10;animation:bannerIn 1.5s ease-out}}
@keyframes bannerIn{{from{{opacity:0;transform:translateY(100%)}}to{{opacity:1;transform:translateY(0)}}}}
.banner-txt{{font-family:{font_family};font-size:clamp(.75rem,1.5vw,1.2rem);background:linear-gradient(135deg,{grad});-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:.12em;text-transform:uppercase}}
.corner-block{{position:absolute;right:0;bottom:clamp(38px,5.5vh,58px);background:rgba(0,0,0,.88);backdrop-filter:blur(24px);border-top-left-radius:clamp(14px,2.2vw,24px);border-left:1px solid rgba(255,255,255,.13);border-top:1px solid rgba(255,255,255,.13);padding:clamp(10px,1.4vw,18px);padding-bottom:0;z-index:11;animation:slideqr 1.2s ease-out;line-height:0;font-size:0}}
.corner-img{{display:block;width:clamp(110px,14vw,200px);height:clamp(110px,14vw,200px);border-radius:clamp(6px,1vw,12px);border:2px solid rgba(255,255,255,.18)}}
...
<div class="stage">
 <div class="canvas">
  ...
  <!-- corner block: QR above banner -->
  <div class="corner-block">
    <img class="corner-img" src="{qr_url}" alt="QR" onerror="this.style.display='none'">
  </div>
  <div class="banner">
    <span class="banner-txt">📸 &nbsp; Compartí tu foto en la pantalla escaneando el QR &nbsp; 📸</span>
  </div>
 </div>
</div>
...
// Ken Burns via requestAnimationFrame — smooth GPU zoom
let kbRaf=null, kbStart=null, kbImg=null;
const KB_DURATION=7000, KB_SCALE=1.06;
function kbStop(){{if(kbRaf){{cancelAnimationFrame(kbRaf);kbRaf=null;}}kbImg=null;}}
function kbStart2(img){{
  kbStop();
  kbImg=img;kbStart=null;
  function step(ts){{
    if(!kbStart)kbStart=ts;
    const p=Math.min((ts-kbStart)/KB_DURATION,1);
    // ease-in-out cubic
    const e=p<0.5?4*p*p*p:(p-1)*(2*p-2)*(2*p-2)+1;
    const s=1+( KB_SCALE-1)*e;
    if(kbImg)kbImg.style.transform='scale('+s.toFixed(5)+')';
    if(p<1&&kbImg)kbRaf=requestAnimationFrame(step);
  }}
  kbRaf=requestAnimationFrame(step);
}}
function initSw(){{if(sw)return;sw=new Swiper('#sw',{{
  loop:true,
  ...
  on:{{
    slideChangeTransitionEnd:function(){{
      ...
      if(img){{
        ...
        if(next){{const ni=next.querySelector('img');if(ni&&ni.dataset.src){{ni.src=ni.dataset.src;}}}}
        kbStart2(img);
      }}
    }},
...
// Wake lock - prevents browser from throttling background tabs
...
# ══════════════════════════════════════════════════════════
# QR OVERLAY PAGE — transparent PNG for use over videos
# ══════════════════════════════════════════════════════════
@app.route('/qr-overlay')
def qr_overlay():
    host       = request.host_url.rstrip('/')
    upload_url = host + '/upload'
    qr_url     = f"https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={upload_url}&bgcolor=00000000&color=ffffff&margin=2&format=png"
    font_k     = gc('font','grande')
    _, font_family = get_font(font_k)
    theme      = gc('theme','fiesta')
    grad = {{'boda':'#E8C4C4,#F9F3EF','cumple':'#FF6B6B,#FFE66D',
            'quince':'#C084FC,#F472B6','empresarial':'#38BDF8,#e2e8f0',
            'fiesta':'#FF3CAC,#FFD700'}}.get(theme,'#FF3CAC,#FFD700')
...
<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@500&display=swap" rel="stylesheet">
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{background:transparent;width:100vw;height:100vh;display:flex;align-items:flex-end;justify-content:flex-end;overflow:hidden}}
.wrap{{
...
.qr-img{{width:200px;height:200px;border-radius:14px;border:2px solid rgba(255,255,255,.2);}}
.url{{margin-top:10px;font-size:.65rem;color:rgba(255,255,255,.3);font-family:'DM Sans',sans-serif;word-break:break-all;}}
.banner{{
...
.hint{{margin-top:20px;font-family:'DM Sans',sans-serif;font-size:.78rem;color:rgba(255,255,255,.35);text-align:center;padding:0 20px;line-height:1.5}}
.btn{{margin-top:14px;padding:10px 22px;background:linear-gradient(135deg,#FF3CAC,#784BA0);border:none;border-radius:8px;color:#fff;font-family:'DM Sans',sans-serif;font-size:.85rem;font-weight:500;cursor:pointer;}}
.btn:hover{{opacity:.85}}
</style></head>
<body>
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;height:100%;padding-bottom:80px;gap:24px">
  <div class="wrap">
    <div class="lbl">COMPARTÍ<br>TU FOTO</div>
    <img class="qr-img" src="{qr_url}" alt="QR">
    <div class="url">{upload_url}</div>
  </div>
  <div class="hint">
    Esta página tiene fondo transparente.<br>
    Para usarla como overlay sobre videos:<br>
    en OBS o tu software de video, agregala como <strong style="color:rgba(255,255,255,.6)">Fuente de Navegador</strong>.
    <br><br>
    O bien hacé una captura de pantalla y guardala como PNG.
  </div>
</div>
<div class="banner">
  <span class="banner-inner">📸 &nbsp; Compartí tu foto en la pantalla escaneando el QR &nbsp; 📸</span>
</div>
</body></html>'''
...
def download_zip():
    ...
    with get_db() as db:
        photos = db.execute(
            "SELECT filename FROM photos WHERE status='approved' ORDER BY sort_order ASC, approved_at ASC"
        ).fetchall()
    if not photos:
        return jsonify({'error': 'No hay fotos aprobadas para descargar'}), 404
    
    # Build zip in memory chunks to avoid timeout
    buf = io.BytesIO()
    added = 0
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_STORED) as zf:
        for i, p in enumerate(photos, 1):
            ...
            if os.path.exists(fp):
                ...
                # Normalize to jpg for compressed files
                arcname = f'foto_{i:03d}.{ext}'
                zf.write(fp, arcname)
                added += 1
    ...
    buf.seek(0)
    event = gc('event_name','PhotoWall').replace(' ','_').replace('/','')
    data = buf.getvalue()
    return Response(
        data,
        mimetype='application/zip',
        headers={
            'Content-Disposition': f'attachment; filename="{event}_fotos.zip"',
