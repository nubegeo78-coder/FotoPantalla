import os, sqlite3, uuid, zipfile, io
...
const KB_DURATION=7000, KB_FROM=1.0, KB_TO=1.07;
function kbStop(){{if(kbRaf){{cancelAnimationFrame(kbRaf);kbRaf=null;}}kbImg=null;}}
function kbStart2(img){{
  kbStop();
  // Set start scale immediately so no jump on fade-in
  img.style.transform='scale('+KB_FROM+')';
  ...
  function step(ts){{
    ...
    const s=KB_FROM+(KB_TO-KB_FROM)*e;
...
function initSw(){{if(sw)return;sw=new Swiper('#sw',{{
  ...
  on:{{
    slideChange:function(){{
      // Reset incoming slide to scale(1) BEFORE the fade starts — no jump
      const next=this.slides[this.activeIndex];
      const nimg=next?next.querySelector('img'):null;
      if(nimg)nimg.style.transform='scale('+KB_FROM+')';
      kbStop();
    }},
