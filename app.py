...
.swiper-slide{{display:flex;align-items:center;justify-content:center;overflow:hidden}}
.swiper-slide img{{width:100%;height:100%;object-fit:contain;transition:none;will-change:transform}}
.swiper-slide.swiper-slide-active img{{animation:kb 8s ease-in-out forwards}}
.swiper-slide.swiper-slide-prev img,.swiper-slide.swiper-slide-next img{{animation:none;transform:scale(1)}}
@keyframes kb{{0%{{transform:scale(1)}}100%{{transform:scale(1.06)}}}}
...
function initSw(){{if(sw)return;sw=new Swiper('#sw',{{
  ...
  on:{{
    slideChangeTransitionEnd:function(){{
      // Reset animation on all slides then restart on active
