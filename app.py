...
.qr{{position:absolute;right:0;bottom:clamp(36px,5.5vh,56px);width:clamp(170px,19vw,285px);background:rgba(0,0,0,.88);backdrop-filter:blur(24px);border-top-left-radius:clamp(14px,2.2vw,24px);border:1px solid rgba(255,255,255,.13);border-right:none;border-bottom:none;padding:clamp(14px,1.8vw,24px);text-align:center;z-index:10;animation:slideqr 1.2s ease-out}}
@keyframes slideqr{{from{{transform:translate(100%,100%);opacity:0}}to{{transform:translate(0,0);opacity:1}}}}
.qr-lbl{{font-family:{font_family};font-size:clamp(1.1rem,2vw,1.65rem);background:linear-gradient(135deg,{grad});-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:.06em;margin-bottom:clamp(8px,1.1vw,14px);line-height:1.25}}
.qr img{{width:100%;max-width:clamp(110px,14vw,200px);border-radius:clamp(6px,1vw,12px);border:2px solid rgba(255,255,255,.18)}}
.qr-url{{margin-top:clamp(4px,.6vw,8px);font-size:clamp(.5rem,.85vw,.75rem);color:rgba(255,255,255,.28);word-break:break-all}}
.banner{{position:absolute;bottom:0;left:0;right:0;height:clamp(36px,5.5vh,56px);background:rgba(0,0,0,.82);backdrop-filter:blur(16px);border-top:1px solid rgba(255,255,255,.08);display:flex;align-items:center;justify-content:center;z-index:10;animation:bannerIn 1.5s ease-out}}
...
def download_zip():
    ...
    with get_db() as db:
        photos = db.execute(
            "SELECT filename FROM photos WHERE status='approved' ORDER BY uploaded_at ASC"
        ).fetchall()
    if not photos:
    ...
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        for i, p in enumerate(photos, 1):
            ...
            if os.path.exists(fp):
                ...
                zf.write(fp, f'foto_{i:03d}.{ext}')
    buf.seek(0)
    event = gc('event_name','PhotoWall').replace(' ','_')
    return Response(
        buf.getvalue(),
        mimetype='application/zip',
        headers={'Content-Disposition': f'attachment; filename={event}_fotos.zip'}
    )
