...
.logo{{font-family:{font_family};font-size:clamp(1.8rem,9vw,3.2rem);background:linear-gradient(135deg,{grad});-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1.15;margin-bottom:4px}}
...
.btn{{width:100%;padding:14px;font-family:{font_family};font-size:1.25rem;letter-spacing:.06em;border:none;border-radius:11px;cursor:pointer;transition:all .3s;background:linear-gradient(135deg,{btn_grad});color:#fff;position:relative;overflow:hidden}}
...
<header>
  ...
  <div class="hr">
    ...
    <a href="/configuracion" class="bs">⚙️ Config</a>
...
async function load(){{const r=await fetch('/api/photos?status='+cur);const photos=await r.json();if(cur==='pending')document.getElementById('cnt').textContent=photos.length+' pendientes';const g=document.getElementById('g-'+cur);g.innerHTML=photos.length?photos.map(p=>card(p,cur)).join(''):`<div class="empty"><span class="empty-i">${{cur==='pending'?'🎉':'📸'}}</span>${{cur==='pending'?'No hay fotos pendientes':'Ninguna foto aprobada aún'}}</div>`;}}
...
def configuracion():
    ...
    for val, label, desc in themes:
        ...
        themes_html += f'''<div class="opt {sel}" onclick="sel(this,'theme','{val}')">
    ...
    for val, fname, label in fonts:
        ...
        fonts_html += f'''<link href="{fu}" rel="stylesheet">
        <div class="opt {sel}" onclick="sel(this,'font','{val}')" style="font-family:{ff}">
          <div class="opt-title" style="font-family:{ff}">{label}</div>
          <div class="opt-desc">{fname}</div>
        </div>'''
...
<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500&display=swap" rel="stylesheet">
{fonts_html.split("</link>")[0] if fonts_html else ""}
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{font-family:'DM Sans',sans-serif;background:#0a0a0f;color:#fff;min-height:100vh}}
body::before{{content:'';position:fixed;inset:0;background:radial-gradient(ellipse at 80% 20%,rgba(255,60,172,.12) 0%,transparent 50%);pointer-events:none}}
header{{position:relative;z-index:1;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.02)}}
.ht{{font-family:'Bebas Neue',sans-serif;font-size:1.4rem;background:linear-gradient(135deg,#FF3CAC,#FFD700);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}}
.bs{{padding:6px 13px;border-radius:7px;border:1px solid rgba(255,255,255,.1);font-size:.8rem;font-weight:500;cursor:pointer;transition:all .2s;background:rgba(255,255,255,.05);color:rgba(255,255,255,.5);font-family:'DM Sans',sans-serif;text-decoration:none;display:inline-block}}
.bs:hover{{color:#fff;background:rgba(255,255,255,.1)}}
.page{{position:relative;z-index:1;max-width:640px;margin:0 auto;padding:22px 18px}}
.section{{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:22px;margin-bottom:18px}}
.section h2{{font-family:'Bebas Neue',sans-serif;font-size:1.2rem;letter-spacing:.06em;margin-bottom:16px}}
label{{display:block;font-size:.82rem;color:rgba(255,255,255,.45);margin-bottom:5px;margin-top:13px}}
label:first-of-type{{margin-top:0}}
input[type=text],input[type=password]{{width:100%;padding:11px 13px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.13);border-radius:9px;color:#fff;font-size:.92rem;font-family:'DM Sans',sans-serif;outline:none;transition:border-color .2s}}
input:focus{{border-color:#FF3CAC}}
.hint{{font-size:.72rem;color:rgba(255,255,255,.22);margin-top:3px}}
.grid2{{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;margin-top:4px}}
.opt{{background:rgba(255,255,255,.04);border:2px solid rgba(255,255,255,.08);border-radius:11px;padding:12px;cursor:pointer;transition:all .2s}}
...
.opt-title{{font-size:.9rem;font-weight:500;margin-bottom:3px}}
.opt-desc{{font-size:.72rem;color:rgba(255,255,255,.35)}}
.bsave{{margin-top:16px;padding:12px 26px;font-family:'Bebas Neue',sans-serif;font-size:1.15rem;letter-spacing:.08em;border:none;border-radius:9px;cursor:pointer;background:linear-gradient(135deg,#FF3CAC,#784BA0);color:#fff;transition:all .3s}}
.bsave:hover{{transform:translateY(-2px);box-shadow:0 6px 18px rgba(255,60,172,.35)}}
.stats{{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px}}
.stat{{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:11px;padding:13px;text-align:center}}
.stat-n{{font-family:'Bebas Neue',sans-serif;font-size:1.9rem;background:linear-gradient(135deg,#FF3CAC,#2B86C5);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}}
.stat-l{{font-size:.72rem;color:rgba(255,255,255,.32);margin-top:2px}}
.danger{{border-color:rgba(255,60,60,.18)}}
.danger h2{{color:#ff6b6b}}
.bclear{{margin-top:4px;padding:12px 26px;font-family:'Bebas Neue',sans-serif;font-size:1.15rem;letter-spacing:.08em;border:none;border-radius:9px;cursor:pointer;background:rgba(255,60,60,.1);border:1px solid rgba(255,60,60,.28);color:#ff6b6b;transition:all .3s}}
.bclear:hover{{background:rgba(255,60,60,.2)}}
.flash-ok{{background:rgba(0,255,150,.07);border:1px solid rgba(0,255,150,.25);color:#00ff96;padding:11px 14px;border-radius:9px;margin-bottom:16px;font-size:.88rem}}
.flash-warn{{background:rgba(255,200,0,.07);border:1px solid rgba(255,200,0,.25);color:#ffd700;padding:11px 14px;border-radius:9px;margin-bottom:16px;font-size:.88rem}}
</style></head>
<body>
<header>
  <div class="ht">⚙️ CONFIGURACIÓN</div>
  <a href="/moderador" class="bs">← Volver al panel</a>
</header>
<div class="page">
  {"<div class='flash-ok'>✅ Configuración guardada</div>" if saved else ""}
  {"<div class='flash-warn'>🗑️ Todas las fotos eliminadas</div>" if cleared else ""}
  ...
  <div class="section danger" style="margin-top:18px">
    <h2>🗑️ Borrar todo al terminar</h2>
    <p style="color:rgba(255,255,255,.4);font-size:.85rem;margin-bottom:14px">Elimina todas las fotos y limpia la base de datos. No se puede deshacer.</p>
