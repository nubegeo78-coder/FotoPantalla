import os, sqlite3, uuid, shutil, zipfile, io
try:
...
function card(p,tab){{
  ...
  const dragAttrs=tab==='approved'?`draggable="true" ondragstart="dragStart(event,${{p.id}})" ondragover="dragOver(event)" ondragleave="dragLeave(event)" ondrop="dragDrop(event,${{p.id}})"`:'' ;
  const dragHandle=tab==='approved'?`<div class="drag-handle" title="Arrastrá para reordenar">⠿ ⠿ ⠿</div>`:'';
  return `<div class="pc" id="c${{p.id}}" ${{dragAttrs}}>
    ${{dragHandle}}
...
async function load(){{
  ...
  // Always refresh counter from server to keep it accurate
...
function dragOver(e){{
  ...
  if(card.id!=='c'+dragId) card.classList.add('drag-over');
}}
function dragLeave(e){{e.currentTarget.classList.remove('drag-over');}}
async function dragDrop(e,targetId){{
  e.preventDefault();
  e.currentTarget.classList.remove('drag-over');
  const el=document.getElementById('c'+dragId);if(el)el.classList.remove('dragging');
...
def approve_photo(pid):
    with get_db() as db:
        ...
        # Get max sort_order so new photo goes to end
        ...
        db.execute('UPDATE photos SET status="approved", approved_at=CURRENT_TIMESTAMP, sort_order=? WHERE id=?',
                   (max_order+1, pid))
        db.commit()
    # Compress on approve — always, not just when oversized
        ...
        try:
            ...
            if os.path.exists(fp):
                ...
                with PILImage.open(fp) as img:
                    ...
                    # Resize if any dimension exceeds 1920px
                    w, h = img.size
                    max_dim = max(w, h)
                    if max_dim > 1920:
                        ratio = 1920 / max_dim
                        img = img.resize((int(w*ratio), int(h*ratio)), PILImage.LANCZOS)
                    # Always re-save as JPEG quality 82 (significant size reduction)
...
function kbStop(){{if(kbRaf){{cancelAnimationFrame(kbRaf);kbRaf=null;}}if(kbImg){{kbImg.style.transform='scale(1)';kbImg=null;}}}}
